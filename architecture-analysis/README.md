...
本文档集基于 `main` 分支提交 `853a80d26`（2026-08-29）进行静态分析。分析对象包括源码、包清单、项目文档、测试入口、CI 和发布脚本；关键结论均回溯到当前源码。

## 核心名词

- `AgentSession`：正式 coding-agent 使用的一次产品会话，连接模型、工具、扩展、设置和磁盘历史。
- `AgentHarness`：`pi-agent-core` 中面向可恢复、可替换存储会话的运行外壳；当前执行调度尚未完成，正式 CLI 不使用它。
...
7. [跨进程协议与工程治理](./07-remote-protocol-and-engineering.md)：protocol/client/server、测试架构、供应链和发布。
...
正式发布的 `pi` 是 `pi-coding-agent` 组装出的编码 Agent：`AgentSession` 负责产品会话，`pi-agent-core` 的 `Agent` 和 agent loop 负责模型—工具循环，`pi-ai` 负责模型供应商接口，`pi-tui` 负责终端 UI。仓库还包含 durable session/compaction 等基础模块、尚未完成的 `AgentHarness` 外壳，以及实验性的 protocol/client/server 跨进程基础设施；当前正式 CLI/SDK 没有经过这些实验组件。
