...
实验性客户端的一次操作先由 client/server 连接传输，再按协议中的路由字段送到目标会话。Chord 解释其中的服务调用并同步服务状态；coding-agent 的 session worker 执行模型和工具。本章先沿这条跨进程路径说明各层职责，再讨论测试、恢复和可观测性。
...
- 默认 pi CLI/SDK 仍创建 `AgentSession`，不经过这条路径。
...
这条路径已接通客户端、服务端和 Harness；默认 CLI/SDK 继续使用 `AgentSession`。
...
连接的第一帧必须是协议握手消息。客户端预先知道逻辑服务器的 `serverId`，握手时验证当前连接的服务端身份。服务器级目标包含 `{ serverId }`；会话级目标还包含 `{ sessionId, attachmentId }`。attachment 表示某个客户端界面与会话的绑定，`attachmentId` 由服务端生成，用于拒绝切换会话后迟到的旧消息。

管理服务的 attach/detach 不直接返回路由目标；服务端通过独立的 attachment 消息发布当前目标。连接断开只释放该界面的绑定，不删除持久会话。客户端会将本地待完成请求标记为失败并清除路由，但不自动重连或重放。调用方需重新连接、重新绑定会话，并根据操作是否可以安全重复来决定是否重试。
...
coordinator 只转发数据，不解释 Pi 消息内容。替换服务端实例时，它断开旧的客户端连接并通知 worker。worker 根据是否仍有会话绑定、是否有活动操作，以及宽限期是否结束来决定退出时间，因此界面断开不会立即终止仍在执行的操作。

## 4. 协议格式与状态同步

每帧由四字节大端长度和一个长度明确的 CBOR 数据项组成，默认上限为 16 MiB。协议校验会拒绝未知的外层字段，并检查消息内容是否符合严格 JSON 数据约束，但不解释具体服务参数。Chord 适配器继续解析服务地址 `{ serviceId, instance?, member, args }`、服务目录、订阅快照与更新，以及服务错误。

服务提供方发布状态变化时，Chord 将变化编码为 Delta 增量。每个订阅独立维护状态路径的编码与解码字典：首次同步发送完整状态，随后按顺序发送增量；提供方替换、断线或重新初始化订阅时重置字典。客户端先安装订阅快照，再调用 `start()` 接收初始化期间缓冲的更新，避免丢失快照与实时更新之间的事件。

`Transcript` 是 Chord 服务，由服务端维护 Harness lane 的权威快照。界面读取完整、不可变的状态副本；外层协议只传输订阅更新，无需定义专用的对话记录消息类型。
...
| Harness 故障矩阵 | faux provider、可暂停或记录调用的测试存储 | 验证外部效果检查点、恢复、重试、工具结果提交和终态事务 |
...
| 失败恢复 | 正式路径有有限 retry/overflow 恢复；Harness 有 durable operation、外部效果检查点、resume 和 replay policy | 两条路径都不承诺外部效果 exactly-once；Harness 仍可能生成 interrupted result |
...
默认产品通过 `AgentSession` 为模型—工具循环加入产品策略；实验路径通过 `AgentHarness` 与 Chord 服务支持可恢复的跨进程运行。两条路径都没有默认权限隔离，也不保证外部效果恰好发生一次；行为评测目前只覆盖部分场景。

## 9. Evaluation：衡量端到端行为
...
比较报告按 eval set 汇总 baseline/candidate 的通过率百分点差，并把 token、延迟和估算费用作为独立的配对差值；`--repetitions` 或 `PI_EVAL_REPETITIONS` 控制比较套件的重复次数。当前覆盖仍窄：仓库尚未形成针对长会话压缩质量、overflow 恢复、危险工具拒绝、不同模型工具成功率、跨平台 shell 行为和故障注入的系统 eval 集。现有结果主要反映上述场景，不能直接代表这些未覆盖任务的质量。
...
Pi 与启动它的本地用户处于同一权限边界。内建 `read`、`write`、`edit` 和 `bash` 可以使用该用户拥有的文件与进程权限；extension 也是同进程受信任代码。
...
工具执行还受响应完整性检查约束：模型响应若因输出 token 限制而截断，Pi 不执行其中任何 tool call。即使残缺参数碰巧能被 JSON 修复并通过 schema，也不能证明它仍代表模型原本意图。
...
- 已完成的 user、assistant 和 tool result 在 `message_end` 时逐条追加到历史；新会话首次 assistant 消息完成时才首次写入文件，随后逐条落盘。

这套策略的共同点是保持明确终态和有限恢复。但正式 `AgentSession` 仍无法保证外部工具效果只发生一次：进程可能在命令已经修改文件、tool result 尚未落盘时崩溃。

`AgentHarness` 接纳操作时先原子记录操作意图。`Drive` 在请求模型或执行工具前写入待执行状态，随后提交流式帧、输出、usage 与下一状态；重试等待、取消请求和最终结果也保存在存储中。恢复时，可安全重放的工具可以继续；不可重放工具若结果不确定，则生成表示执行中断的结果。外部修改或模型计费可能已经发生，存储事务无法撤销这些效果，因此仍不保证 exactly-once。
...
5. Harness 的 operation/recovery 实现与待实现部分见 [`AgentSession` 与 `AgentHarness`](./05-agent-session-and-execution.md#41-具体实现)。
...
`Agent` 顺序等待自身订阅者，扩展 hook 也按约定顺序执行，使内部状态更新与消息追加完成后再推进循环。`AgentSession.subscribe()` 对宿主的公开通知只同步调用回调，不等待回调返回的 Promise；宿主异步任务的完成顺序需要自行管理。
...
正式 coding-agent 当前提供的观测数据包括 `AgentSessionEvent`、每条 assistant usage、工具进度、session JSONL 和 eval artifact。费用与 cache 统计直接从 provider usage 派生，不根据本地猜测宣称 cache hit。

`pi-telemetry` 还定义了 provider request、operation、turn、retry step、tool、hook、event handler 和 session write 等 span 契约。当前已落地的是显式 `Context` 传播、typed span helper，以及 `before_tool`/`after_tool` 的部分 hook span；大多数 Drive、事件处理和会话写入位置尚未创建对应 span。正式 `AgentSession` 也没有自动产生这些 Harness span，跨进程 trace carrier 注入/提取尚未实现，不能把 schema 等同于完整分布式 trace。

另一个容易混淆的机制是 install telemetry：它是可配置的安装/版本报告，不等于上述 Agent 运行 trace。

性能优化分别作用于模型请求、工具输出和终端渲染：provider prompt cache 复用稳定前缀；API 实现延迟加载减少启动成本；工具输出截断控制 context；TUI 差量渲染减少终端写入；并行工具降低独立调用的等待时间。这些优化都没有改变 agent loop 的基本语义。
...
4. **明确隔离与恢复的限制**：本地工具默认拥有用户权限，真正隔离交给 OS；正式产品只做有限 retry 和消息级持久化；Harness 提供 durable recovery，但明确不把它宣传成外部效果 exactly-once。

定位问题时，可以先按这些职责找到控制循环、产品会话、模型适配或存储入口，再沿前文的调用链检查事件和状态变化。
